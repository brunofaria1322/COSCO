class MyContainer:
    ...
    def getApparentIPS(self):
        ...
        canUseIPS = (hostIPSCap - hostBaseIPS) / len(
            self.env.getContainersOfHost(self.hostid)
        )
        if canUseIPS < 0:
            return 0
    ...
    def allocate(self, hostID, allocBw):
        ...
        if self.hostid != hostID:
            ...
            lastMigrationTime += abs(
                self.env.hostlist[self.hostid].latency
    ...
    def execute(self, lastMigrationTime):
        ...
        requiredExecTime = (
            (self.ipsmodel.totalInstructions - self.ipsmodel.completedInstructions)
            / apparentIPS
            if apparentIPS
            else 0
        )
        self.totalExecTime += min(execTime, requiredExecTime)
        self.ipsmodel.completedInstructions += apparentIPS * min(
            execTime, requiredExecTime
        )
