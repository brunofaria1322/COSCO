...
def createfiles(df):
    ...
    for i in tqdm(range(1, 501), ncols=80):
        ...
        pd.DataFrame(trace).to_csv(
            f"simulator/workload/datasets/azure_2019/{i}.csv", header=False, index=False
        )
...
class MyAzure2019Workload(Workload):
    def __init__(self, numContainers):
        ...
        self.csv_data = [[], []]  # 2 types [edge, fog]
        ...
        if path.exists(f"{possible_path}{self.meanSLA}-{self.sigmaSLA}.pkl"):
            ...
            cpus_ips = [2048, 12 * 2048]  # minimum - from MyFog
            rams = [2 * 1024, 48 * 1024]  # minimum - from MyFog
            max_containers = [10, 40]  # manually   
            # each fog can support a minimum of 4 childs

            all_ips = []
            for i in range(1, 500):
                df = pd.read_csv(
                    self.dataset_path + "rnd/" + str(i) + ".csv", sep=";\t"
                )
                ...
                if (
                    ...
                    and temp_ram < rams[0] / max_containers[0]
                ):
                ...
                if (
                    ...
                    and temp_ram < rams[1] / max_containers[1]
                ):
    ...
    def generateNewContainers(self, interval, hosts):
        #
        # layer_type:	0 - edge
        # 				1 - fog
        # 				2 - cloud
...
def test1():
    ...
    all_rams = np.array(
        [np.median(df["Memory usage [KB]"].to_numpy()[:max_sla]) for df in df_list]
    )
    all_ips = np.array(
        [
            np.max(
                df["CPU capacity provisioned [MHZ]"].to_numpy()[:max_sla]
                * df2.to_numpy()[:max_sla, 0]
                / 100
            )
            for df, df2 in zip(df_list, df2_list)
        ]
    )
    ...
    def compute_indexes(
        rams, cpus, mul_cpu, mul_ram, min_ram, max_ram, min_cpu, max_cpu
    ):
        indexes = [0, 0, 0]
        indexes[0] = np.sum(
            rams[min_ram[0] < rams / mul_ram < max_ram[0]]
            & cpus[min_cpu[0] < cpus / mul_cpu < max_cpu[0]]
        )
        indexes[1] = np.sum(
            rams[min_ram[1] < rams / mul_ram < max_ram[1]]
            & cpus[min_cpu[1] < cpus / mul_cpu < max_cpu[1]]
        )
        indexes[2] = np.sum(
            rams[min_ram[2] < rams / mul_ram < max_ram[2]]
            & cpus[min_cpu[2] < cpus / mul_cpu < max_cpu[2]]
        )
        return indexes
    ...
    plt.xlabel("mul_ram")
    plt.xticks(
        np.linspace(0, num_values, 5), np.linspace(bounds[1][0], bounds[1][1], 5)
    )
    plt.ylabel("mul_cpu")
    plt.yticks(
        np.linspace(0, num_values, 5), np.linspace(bounds[0][0], bounds[0][1], 5)
    )
...
def test2():
    ...
    containers1 = [
        [
            max(1, round(gauss(dur_mean, dur_std)))
            for _ in range(max(1, poisson(arr_mean)))
        ]
    ]
    containers2 = [[]]
    containers3 = [[]]
    ...
    for i in range(1, num_sim):
        ...
        containers1[i].extend(
            [
                max(1, round(gauss(dur_mean, dur_std)))
                for _ in range(max(1, poisson(arr_mean)))
            ]
        )
        containers2[i].extend(
            [
                max(1, round(gauss(dur_mean, dur_std)))
                for _ in range(containers1[i - 1].count(1))
            ]
        )
        containers3[i].extend(
            [
                max(1, round(gauss(dur_mean, dur_std)))
                for _ in range(containers2[i - 1].count(1))
            ]
        )
